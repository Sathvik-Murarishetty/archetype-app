...
export default function HobbyistsBingoCard() {
    ...
    useEffect(() => {
        const saved = localStorage.getItem("hobbyist_checked");
        const completed = localStorage.getItem("hobbyist_complete");
        const hasSeenPopup = localStorage.getItem("hobbyist_popup_seen");
        ...
        if (completed === "true") {
            ...
            if (!hasSeenPopup) {
                ...
                localStorage.setItem("hobbyist_popup_seen", "true");
            }
        }
    }, []);

    useEffect(() => {
        localStorage.setItem("hobbyist_checked", JSON.stringify(Array.from(checked)));
        const isComplete = checked.size === totalTasks;
        if (isComplete && !complete) {
            setComplete(true);
            localStorage.setItem("hobbyist_complete", "true");
            localStorage.setItem("hobbyist_popup_seen", "true");
            setTimeout(() => setShowPopup(true), 300);
        }
    }, [checked]);
    ...
    const handleReset = () => {
        localStorage.removeItem("hobbyist_checked");
        localStorage.removeItem("hobbyist_complete");
        localStorage.removeItem("hobbyist_popup_seen");
        setChecked(new Set());
        setComplete(false);
        setShowPopup(false);
    };
